...
def research(query):
    ...
    researcher = autogen.AssistantAgent(
        name="researcher",
        system_message="Research about a given query, collect as much relevant information as possible using the scraping function. generate detailed research results with alot of technical details with all reference links attached; Add TERMINATE to the end of the research report;",
...
def write_content(research_material, topic):
    editor = autogen.AssistantAgent(
        name="editor",
        system_message="You are a senior editor of an AI blogger, you will define the structure of a 1300 word blog post based on material provided by the researcher, and give it to the writer to write the blog post",
    ...
    writer = autogen.AssistantAgent(
        name="writer",
        system_message="You are a professional AI blogger who is writing a blog post about AI,the blog post will be around  words long. you will write a short blog post based on the structured provided by the editor, and feedback from reviewer; After 2 rounds of content iteration, add TERMINATE to the end of the message",
        llm_config={"config_list": config_list},
    )
...
user_proxy.initiate_chat(
    writing_assistant, message="write a blog about AI Agents and case uses in a variety of businesses")
